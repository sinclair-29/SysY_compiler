...
void DFA::terminate() {
	...
	// do nothing
	if(this->current_state == &DFA::state_single) {
		return;
	}
...
bool DFA::is_comment(char ch) {
	if( this->current_state == &DFA::state_comment1 ||
		this->current_state == &DFA::state_comment2
		) return true;
	if( (ch == ' ' || ch == '\t') && this->current_state == &DFA::state_single
		) return true;
	return false;
}
...
bool DFA::state_division(char next) {
	...
	if(next == '/') {
		//std::cout << "move to single" << std::endl;
		this->current_state = &DFA::state_single;
		return true;
	}
	return false;
}

bool DFA::state_single(char next) {
	if(next != '\n') return true;
	return false;
}
